...
def process_upload(uploaded_file, model_path: Path, classes: list[str], confidence: float, line_fraction: float):
...
if uploaded_file is not None:
    ...
    if st.button("Process video", type="primary"):
        with st.spinner("Detecting, tracking, and computing video analytics. This can take a few minutes on CPU."):
            try:
                output_bytes = process_upload(uploaded_file, selected_model, classes, confidence, line_fraction)
